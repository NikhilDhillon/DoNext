...
import { OutlineReviewQueue } from "@/components/outline-review-queue";
    ...
    updateProposal(proposalIndex, {
      ...
      schemes: proposal.schemes.map((scheme, currentSchemeIndex) =>
        currentSchemeIndex === schemeIndex
          ? (() => {
            const components = scheme.components.map((component, currentComponentIndex) =>
    ...
    <>
      ...
      <div className="outline-proposals" ref={proposalsRef}>
        {proposals.map((proposal, proposalIndex) => {
          ...
          return <OutlineReviewQueue
            busy={busy}
            existingCourse={existingCourse}
            key={`${proposal.source_files.join("-")}-${proposalIndex}`}
            proposal={proposal}
            semesterEndDate={semester.end_date}
            onChange={(nextProposal) => updateProposal(proposalIndex, nextProposal)}
            onDismiss={() => setProposals((current) => current.filter((_, index) => index !== proposalIndex))}
            onImport={() => void importProposal(proposalIndex)}
            onRemoveItem={(itemIndex) => removeProposalItem(proposalIndex, itemIndex)}
            onUpdateSchemeComponent={(schemeIndex, componentIndex, updates) => updateSchemeComponent(proposalIndex, schemeIndex, componentIndex, updates)}
          />;
        })}
      </div>
      ...
      {!proposals.length && showManualEntry ? <details className="manual-entry-panel">
        ...
        <div className="manual-entry-content">
          <form className="onboarding-form compact-form" onSubmit={onCreateCourse}>
            ...
            <div className="form-row"><label><span>Delivery</span><select name="delivery_mode" defaultValue="scheduled"><option value="scheduled">Scheduled classes</option><option value="asynchronous">Asynchronous</option></select></label><label><span>First content available <small>For asynchronous courses</small></span><input name="first_content_available_at" type="datetime-local" /></label></div>
          ...
          {courses.length ? <form className="onboarding-form compact-form" onSubmit={onCreateItem}>
            <div className="form-row"><label><span>Course</span><select name="course_id" required>{courses.map((course) => <option key={course.id} value={course.id}>{course.code}</option>)}</select></label><label><span>Type</span><select name="item_type" defaultValue="assignment"><option value="assignment">Assignment</option><option value="quiz">Quiz</option><option value="midterm">Midterm</option><option value="final_exam">Final exam</option><option value="project">Project</option><option value="lab">Lab</option><option value="presentation">Presentation</option><option value="other">Other</option></select></label></div>
            <label><span>Name</span><input name="name" placeholder="Assignment 1" required /></label>
            <div className="form-row"><label><span>Deadline</span><input name="deadline" type="date" required /></label><label><span>Grade weight <small>Optional</small></span><input name="weight_percent" min="0" max="100" step="0.5" type="number" /></label></div>
            <label className="checkbox-field"><input name="required" type="checkbox" defaultChecked /><span><strong>Required item</strong><small>Optional work is the first academic work omitted during overload.</small></span></label>
