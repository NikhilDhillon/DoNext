...
def test_midterm_is_scheduled_as_labeled_preparation_across_preferred_days(
    ...
    assert proposal["generation_summary"]["exam_preparation"][0]["scheduled_prep_minutes"] == 200
    assert all(block["title"] != "Midterm Exam" for block in academic_blocks)
    assert all(block["title"] == "CSC 370 · Midterm prep" for block in academic_blocks)
